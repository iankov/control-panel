        ...
        <section class="sidebar">
            <ul class="sidebar-menu" data-widget="tree" data-accordion="0" data-animation-speed="200">
                ...
                @foreach(collect(config('icp-menu.groups')) as $key => $group)
                    @if(empty($group['items']) || !array_get($group, 'visible', config('icp-menu.default-group-visibility')))
                        @continue
                    @endif
                    <li class="treeview menu-open">
                        ...
                        <ul class="treeview-menu" style="display: block;">
                            @foreach(collect($group['items']) as $menu)
                                @if(!array_get($menu, 'visible', config('icp-menu.default-item-visibility')))
                                    @continue
                                @endif
                                <li>
                                    <a href="{{array_get($menu, 'link', '#')}}">
